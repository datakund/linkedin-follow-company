...
A python library that uses browser automation to login & follow company page on linkedin
...
```sh
pip install linkedin-follow-company
```

### Import

```javascript
from linkedin-follow-company import *
```
...
To follow company page on linkedin we use ``follow_company`` method.
It requires **company_link** as input parameter

```javascript
linkedin.follow_company(company_link="company_linkedin_page_link_here")
```
